...
chronology.fine<-trap.chronology(new.dat,event.time.threshold=30,use.mlv.thershold=TRUE,use.stat.fudge=TRUE, fudge.user=4)
...
trap.harvest.chronology<-assign.orphen.events(trap.harvest.chronology, max.time=20.5)
...
events.1day <- date.lut[date.lut$start.date==date.lut$end.date,c("AGRP_PRP_ID","unk.prp.event.id","ALWS_AGRPROP_ID")]
nrow(trap.harvest.chronology)
...
trap.harvest.chronology <- break.up.long.events(trap.harvest.chronology, long.event.thershold=30)
...
nrow(trap.harvest.chronology[trap.harvest.chronology$Orphen.Flag=="Orphen" & trap.harvest.chronology$day.diff==0,])
sum(trap.harvest.chronology[trap.harvest.chronology$Orphen.Flag=="Orphen" & trap.harvest.chronology$day.diff==0,"take"])
...
nrow(day.diff.vec<-trap.harvest.chronology[trap.harvest.chronology$Orphen.Flag=="Orphen" & trap.harvest.chronology$day.diff<=5 & trap.harvest.chronology$day.diff>0,])
...
trap.harvest.chronology[trap.harvest.chronology$event.type=="Single Day Event","days.active"] <- 1
...
final.agg.out.dat <- final.agg.out.dat[,c("AGRP_PRP_ID","unk.prp.event.id","ALWS_AGRPROP_ID","ST_NAME","CNTY_NAME", "ST_GSA_STATE_CD", "CNTY_GSA_CNTY_CD", "COUNTY.OR.CITY.LAND","MILITARY.LAND","PRIVATE.LAND","STATE.LAND","TRIBAL.LAND","TOTAL.LAND","CMP_NAME", "start.date","end.date", "event.length", "trap.nights", "Take", "trap.night.certainty")]
...
trap.harvest.chronology.limited <- trap.harvest.chronology[trap.harvest.chronology$unk.prp.event.id %in% remove.vec,]
