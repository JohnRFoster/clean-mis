...
if(nrow(in.dat)!=0){
  ...
  freq.prop<-count(in.dat$AGRP_PRP_ID)
  ...
  if(nrow(in.dat)!=0){
    ...
    agrp_prp.list <- count(in.dat$AGRP_PRP_ID)
    ...
    for(i in 1:length(prop.list)){
      ...
      if(sum(tmp.dat[,c("SET","CHECKED","REMOVED")])!=0){
        ...
        if(use.mlv.thershold==TRUE){
          ...
          ml.value <- mlv(tmp.dat$day.diff, method="mfv", na.rm=TRUE)
          
          if(length(ml.value)>1){
            ml.value <- mlv(tmp.dat$day.diff, method="Venter", na.rm=TRUE)
          }
