...
make.property.lut<-function(dat.Agr){
  ...
  lut.property.acres <- generate.mean.property.size(lut.property.acres, area.col="TOTAL.LAND",
                                                   area.thershold=2, adj.file="C:/DATA/NASS/county_adjacency2010.csv")
...
generate.mean.property.size <- function(in.dat, area.col, area.thershold, adj.file="C:/DATA/NASS/county_adjacency2010.csv"){
  ...
  for(i in 1:length(fips.vec)){
    ...
    vec<-in.dat[in.dat$FIPS == fips.vec[i],"TOTAL.LAND"]
    ...
    if(length(vec)>0){
      mu <- mean(vec)
      st.dev <- sd(vec)
      n <- length(in.dat[in.dat$FIPS == fips.vec[i] & in.dat$TOTAL.LAND <= area.thershold,area.col])
    }#END Logical
    
    #--Mean using adjacent counties
    if(length(vec)==0){
      fips.lut<-adj.file[adj.file$FIPS %in% fips.vec[i],"FIPS.neighbor"]
      
      vec<-in.dat[in.dat$FIPS %in% fips.lut, area.col]
      vec<-vec[vec>area.thershold]
      vec<-vec[is.na(vec)==FALSE]
